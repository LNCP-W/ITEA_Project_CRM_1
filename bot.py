...
def start(message):
    bot.send_message(chat_id=message.chat.id, text="Привет. Это бот сервисного центра 'Очень крутой сервисный центр'")
    markup = types.ReplyKeyboardMarkup(row_width=2)
    button1 = types.KeyboardButton('Да', request_contact=True)
    button2 = types.KeyboardButton('Нет')
    markup.add(button1, button2)
    bot.send_message(chat_id=message.chat.id, text="Хотите подписатся на рассылку уведомлений?", reply_markup=markup)
